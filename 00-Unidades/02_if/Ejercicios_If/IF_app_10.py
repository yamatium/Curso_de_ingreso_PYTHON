...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        mensaje = f"la nota es {numInt}"

        if (numInt >= 6):
            promocion = f"Promoción directa, "
            alert("UTN", promocion + mensaje)

        elif (numInt == 5 or numInt == 4):
            aprobado = f"Aprobado, "
            alert("UTN", aprobado + mensaje)

        elif (numInt <= 3):
           desaprovado = f"Desaprobado, "
           alert("UTN", desaprovado + mensaje)
